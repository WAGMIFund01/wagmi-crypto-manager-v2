...
import PersonalAddAssetForm from '@/features/transactions/components/PersonalAddAssetForm';
...
interface PersonalPortfolioOverviewProps {
  className?: string;
  onRefresh?: () => void;
  isPrivacyMode?: boolean;
}

export default function PersonalPortfolioOverview({ className, onRefresh, isPrivacyMode = false }: PersonalPortfolioOverviewProps) {
  ...
  const fetchPortfolioData = async () => {
    try {
      console.log('🔄 Fetching personal portfolio data...');
      setLoading(true);
      setError(null);
      const response = await fetch('/api/get-personal-portfolio-data');
      const data = await response.json();

      console.log('📊 Personal portfolio data response:', {
      ...
      if (data.success) {
        ...
        console.log('✅ Personal portfolio data updated successfully');
      } else {
        setError(data.error || 'Failed to fetch personal portfolio data');
        console.log('❌ Personal portfolio data fetch failed:', data.error);
      }
    } catch (err) {
      console.error('❌ Error fetching personal portfolio data:', err);
      setError('Failed to fetch personal portfolio data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditAssetSave = async (editData: {
    ...
    try {
      console.log('=== PersonalPortfolioOverview: handleEditAssetSave called ===');
      console.log('Edit data received:', editData);
      
      const response = await fetch('/api/edit-personal-asset', {
        method: 'PUT',
        headers: {
  ...
  const handleRemoveAsset = async (symbol: string) => {
    ...
    if (!confirm(`Are you sure you want to remove ${symbol} from your personal portfolio?`)) {
    ...
    try {
      ...
      const response = await fetch(`/api/remove-personal-asset?symbol=${encodeURIComponent(symbol)}`, {
        method: 'DELETE',
      });
    ...
    return (
      <div className={`${className} flex items-center justify-center min-h-96`}>
        <WagmiSpinner 
          size="lg" 
          theme="green" 
          text="Loading personal portfolio data..." 
          showText={true} 
          centered={true}
        />
      </div>
    ...
    return (
      <div className={`${className} flex items-center justify-center min-h-96`}>
        <div className="text-center">
          ...
          <p className="text-red-400 mb-2">Error loading personal portfolio data</p>
    ...
    <div className={`${className} space-y-6`}>
      ...
      <WagmiCard variant="container" theme="green" size="lg" className="overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-700 flex justify-between items-center">
          <WagmiText variant="h4" weight="semibold" color="primary">Personal Portfolio Assets</WagmiText>
          <WagmiButton
            variant="primary"
